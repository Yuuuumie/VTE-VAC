...
from config import IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD, cfg
...
class CoTHybridNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000, in_chans=3,
                 ...
                 drop_block_rate=0., global_pool='avg', zero_init_last_bn=True, block_args=None, layer_num=4):
        ...
        if cfg.modal == ['rgb' ,'roi', 'ranking']:
            self.global_pool, self.fc, self.proposal_fc, self.regression_fc = create_classifier(self.num_features, self.num_classes, pool_type=global_pool)
        elif cfg.modal == ['rgb' ,'roi']:
            self.global_pool, self.fc, self.proposal_fc = create_classifier(self.num_features, self.num_classes, pool_type=global_pool)
        elif cfg.modal == ['roi', 'ranking']:
            self.global_pool, self.proposal_fc, self.regression_fc = create_classifier(self.num_features, self.num_classes, pool_type=global_pool)
        elif cfg.modal == ['rgb' ,'ranking']:
            self.global_pool, self.fc, self.regression_fc = create_classifier(self.num_features, self.num_classes, pool_type=global_pool)
        else:
            raise AssertionError(f"unexpected modal list {cfg.modal}")
    ...
    def get_proposal_classifier(self):
        return self.proposal_fc
    ...
    def reset_classifier(self, num_classes, global_pool='avg'):
        ...
        if cfg.modal == ['rgb' ,'roi', 'ranking']:
            ...
            raise AssertionError("unexpected modal list")
    ...
    def forward(self, x):
        ...
        if 'rgb' in cfg.modal and 'roi' in cfg.modal and 'ranking' in cfg.modal:
            output_x = self.fc(x)
            proposal_x = self.proposal_fc(x)
            regression_x = self.regression_fc(x)
            return output_x, proposal_x, regression_x   

        elif 'rgb' in cfg.modal and 'roi' in cfg.modal and 'ranking' not in cfg.modal:
            output_x = self.fc(x)
            proposal_x = self.proposal_fc(x)
            return output_x, proposal_x 

        elif 'rgb' not in cfg.modal and 'roi' in cfg.modal and 'ranking' in cfg.modal:
            proposal_x = self.proposal_fc(x)
            regression_x = self.regression_fc(x)
            return proposal_x, regression_x   
            
        elif 'rgb' in cfg.modal and 'roi' not in cfg.modal and 'ranking' not in cfg.modal:
            output_x = self.fc(x)
            regression_x = self.regression_fc(x)
            return output_x, regression_x           
        else:
