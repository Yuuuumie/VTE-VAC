...
class TrainMeter(object):
    ...
    def __init__(self, start_epoch, num_epochs, epoch_iters):
        ...
        self.loss1 = ScalarMeter(cfg.solver.log_interval)
        self.loss2 = ScalarMeter(cfg.solver.log_interval)
    ...
    def update_stats_2d(self, loss1, loss2, lr, mb_size):
        self.loss1.add_value(loss1)
        self.loss2.add_value(loss2)
        self.lr = lr
        self.loss_total += (loss1+loss2) * mb_size
        self.num_samples += mb_size
    ...
    def get_iter_stats_2d(self, cur_epoch, cur_iter):
        cur_iter_total = (cur_epoch - self.start_epoch) * self.epoch_iters + cur_iter + 1
        eta_sec = self.iter_timer.average_time * (self.max_iter - cur_iter_total)
        ...
        stats = {
            "epoch": "{}/{}".format(cur_epoch + 1, self.max_epoch),
            "iter": "{}/{}".format(cur_iter + 1, self.epoch_iters),
            "loss1": self.loss1.get_win_avg(),
            "loss2": self.loss2.get_win_avg(),
            "lr": self.lr
        }
        return stats
    ...
    def log_iter_stats(self, cur_epoch, cur_iter):
        ...
        info = "Epoch: {:s}, Iter: {:s}, loss: {:.4f}, lr: {:.8s}".format(\
            stats["epoch"], stats["iter"], stats["loss"], stats["lr"])
        logger_info(info)
    ...
    def log_iter_stats_2d(self, cur_epoch, cur_iter):
        if (cur_iter + 1) % cfg.solver.log_interval != 0:
            return
        stats = self.get_iter_stats_2d(cur_epoch, cur_iter)
        info = "Epoch: {:s}, Iter: {:s}, loss1: {:.4f}, loss2: {:.4f}, lr: {:.6s}".format(\
            stats["epoch"], stats["iter"], stats["loss1"], stats["loss2"], stats["lr"])
        logger_info(info)
...
class TestMeter_SFB(object):
    ...
    def update_stats(self, acc, mb_size):
        self.acc += acc
        self.num_samples += mb_size

    def log_iter_stats(self, cur_epoch):
        if cfg.distributed:
            tensor_reduce = torch.tensor([self.acc * 1.0, self.num_samples * 1.0], device="cuda")
            tensor_reduce = sum_tensor(tensor_reduce)
            tensor_reduce = tensor_reduce.data.cpu().numpy()
            acc = tensor_reduce[0]
        ...
        acc = acc* 1.0 / num_samples
        info = "Epoch: {:d}, acc = {:.2%} in {:d}".format(cur_epoch + 1, acc, int(num_samples))
        logger_info(info)
        return acc
