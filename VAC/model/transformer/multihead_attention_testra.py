...
class TesTra_DotProductAttention(nn.Module):

    def __init__(self, dropout=0.0, alpha=1.0, num_heads=8):
        super(TesTra_DotProductAttention, self).__init__()
...
class TesTra_DotProductAttentionStream(TesTra_DotProductAttention):

    def __init__(self, dropout=0.0, alpha=1.0):
        super(TesTra_DotProductAttentionStream, self).__init__(dropout, alpha)
...
class TesTra_MultiheadAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0.0, bias=True, kdim=None, vdim=None,
        ...
        if attention_type == 'dotproduct':
            self.attention = TesTra_DotProductAttention(dropout, decay_alpha, num_heads)
        elif attention_type == 'linear':
            self.attention = TesTra_LinearAttention(dropout)
        else:
...
class TesTra_MultiheadAttentionStream(TesTra_MultiheadAttention):

    def __init__(self, embed_dim, num_heads, dropout=0.0, bias=True, kdim=None, vdim=None,
                 attention_type='dotproduct', decay_alpha=1.0):
        super(TesTra_MultiheadAttentionStream, self).__init__(embed_dim, num_heads,
                                                       dropout, bias, kdim,
                                                       vdim, attention_type,
                                                       decay_alpha)

        if attention_type == 'dotproduct':
            self.attention = TesTra_DotProductAttentionStream(dropout, decay_alpha)
        elif attention_type == 'linear':
            self.attention = TesTra_LinearAttentionStream(dropout)
        else:
            raise RuntimeError('attention_type should be [dotproduct | linear]')
...
class TesTra_LinearAttention(nn.Module):

    def __init__(self, dropout=0.0, nonlinear=F.elu,
                 eps=1e-6):
        super(TesTra_LinearAttention, self).__init__()
    ...
    def forward(self, q, k, v, attn_mask=None):
        ...
        #     raise RuntimeError(("TesTra_LinearAttention does not support arbitrary "
        #                         "attention masks"))

        return attn_output

class TesTra_LinearAttentionStream(TesTra_LinearAttention):

    def __init__(self, dropout=0.0):
        super(TesTra_LinearAttentionStream, self).__init__(dropout)
